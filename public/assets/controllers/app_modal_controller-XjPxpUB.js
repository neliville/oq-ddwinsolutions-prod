...
/**
 * Contrôleur Stimulus pour les modales (Tailwind, sans Bootstrap)
 * Affichage via style.display + backdrop .tw-backdrop
...
export default class extends Controller {
    ...
    connect() {
            ...
            requestAnimationFrame(() => {
               ...
               this.boundCloseModal = (event) => {
                   ...
                   if (!modalId || this.element.id === modalId) {
                       console.log('[app-modal] modal:close event received', {
                           modalId: modalId || 'all',
        ...
        this.boundOpenModal = (event) => {
            console.log('[app-modal] modal:open event received', {
            ...
            const modalId = event.detail?.modalId;
            if (modalId && this.element.id === modalId) {
                console.log('[app-modal] Modal ID matches, opening modal');
                ...
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        if (this.element && document.contains(this.element)) {
                            console.log('[app-modal] Opening modal directly');
                            this.show();
                        } else {
                            console.warn('[app-modal] Element not in DOM yet, waiting...');
                            // Réessayer après un délai supplémentaire
                            setTimeout(() => {
                                if (this.element && document.contains(this.element)) {
                                    console.log('[app-modal] Opening modal after retry');
                                    this.show();
                                } else {
                                    console.error('[app-modal] Element still not in DOM after retry');
                                }
                            }, 300);
                        }
                    }, 50);
                ...
                console.log('[app-modal] Modal ID does not match', {
                    modalId,
                    elementId: this.element.id
                });
            }
        ...
        document.addEventListener('modal:open', this.boundOpenModal, true);
               ...
               this.boundTurboRender = (event) => {
                   console.log('[app-modal] Turbo/Live render event received', {
                       type: event.type,
                       detail: event.detail,
               ...
               this.boundLiveActionSuccess = (event) => {
                   console.log('[app-modal] Live action success event received', {
                       type: event.type,
                       detail: event.detail,
                       timestamp: new Date().toISOString()
                   });
        ...
        this.boundButtonClick = (event) => {
            ...
            if (modalId === 'categoryModal' && (actionParam === 'openEditCategoryModal' || actionParam === 'openAddCategoryModal')) {
                ...
                console.log('[app-modal] Opening modal directly after button click', {
                    actionParam,
                    modalId,
                    categoryId
                });
        ...
        this.boundTurboBeforeRender = (event) => {
            console.log('[app-modal] Turbo before render event received', {
                type: event.type,
                detail: event.detail,
                timestamp: new Date().toISOString()
            });
        ...
        this.boundLiveComponentUpdate = (event) => {
            console.log('[app-modal] Live Component update event received', {
                type: event.type,
                detail: event.detail,
                timestamp: new Date().toISOString()
            });
        ...
        this.boundTurboFrameLoad = () => {
            console.log('[app-modal] Turbo frame load event received');
            setTimeout(() => {
                this.checkAndOpenModal();
            }, 150);
        };
            ...
            this.observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'data-live-props-value') {
                        ...
                        console.log('[app-modal] Live props changed detected by MutationObserver', {
                            newValue: newValue,
                            timestamp: new Date().toISOString()
                        });
        ...
        this.boundFormSubmit = (event) => {
            ...
            if (hiddenInput && hiddenInput.value) {
                ...
                console.log('[app-modal] Updated data-live-action-param-editing-category-id from hidden input', {
    ...
    /**
     * Valeur de display à l'ouverture : les overlays Tailwind centrés utilisent
     * `flex items-center justify-center` sur la racine ; `display: block` les cassait
     * (contenu collé en haut à gauche).
     */
    getOpenDisplayValue() {
        if (!this.element) return 'block';
    ...
    show() {
        ...
        this.element.style.display = this.getOpenDisplayValue();
        ...
        requestAnimationFrame(() => {
            ...
            this.element.dispatchEvent(new CustomEvent('app-modal:shown', { bubbles: true }));
        });
    }
    ...
    hide() {
        ...
        this.element.dispatchEvent(new CustomEvent('app-modal:hidden', { bubbles: true }));
    }
        ...
        try {
            ...
            if (modalId === 'categoryModal') {
                ...
                if (editingCategoryId !== null && props.categories && Array.isArray(props.categories)) {
                    ...
                    console.log('[app-modal] updateModalContent - categoryModal', {
                        editingCategoryId,
                        editingCategory,
            ...
            console.warn('[app-modal] Error parsing Live props for modal content update:', e);
        }
    }
    ...
    checkAndOpenModal() {
        ...
        const liveComponent = document.querySelector('[data-controller*="live"]');
        if (!liveComponent) {
            console.debug('[app-modal] No Live Component found');
            return;
        }
        ...
        const propsValue = liveComponent.getAttribute('data-live-props-value');
        if (!propsValue) {
            console.debug('[app-modal] No Live props found');
            return;
        }
        ...
        try {
            ...
            console.log('[app-modal] Checking modal', {
                modalId,
                props,
                ...
                showCategoryModal: props.showCategoryModal
                   ...
                   if (modalId === 'categoryModal') {
                       ...
                       console.log('[app-modal] Category modal check', {
                           hasEditingId,
                           showModal,
                           shouldOpen,
                           isVisible: this.isVisible(),
                       ...
                       if (shouldOpen && !this.isVisible()) {
                           console.log('[app-modal] Opening category modal from props check');
                           this.updateModalContent(); // Mettre à jour le contenu avant d'ouvrir
                           this.show();
                       } else if (!shouldOpen && this.isVisible()) {
                           console.log('[app-modal] Closing category modal from props check');
            ...
            if (modalId === 'causeModal') {
                ...
                console.log('[app-modal] Cause modal check', {
                    ...
                    isVisible: this.isVisible()
                       ...
                       if (shouldOpen && !this.isVisible()) {
                           console.log('[app-modal] Opening cause modal from props check');
                           this.updateModalContent(); // Mettre à jour le contenu avant d'ouvrir
                           this.show();
                       } else if (!shouldOpen && this.isVisible()) {
                           console.log('[app-modal] Closing cause modal from props check');
                           this.hide();
                       }
            }
        } catch (e) {
            console.warn('[app-modal] Error parsing Live props:', e);
        }
    }
}
