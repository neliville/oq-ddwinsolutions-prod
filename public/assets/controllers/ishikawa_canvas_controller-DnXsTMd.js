...
export default class extends Controller {
    ...
    async exportPDF() {
        ...
        const imgY = marginTop + (availableHeight - imgHeight * ratio) / 2;

        pdf.addImage(imgData, 'PNG', imgX, imgY, imgWidth * ratio, imgHeight * ratio);

        const titleText = (this.diagramDataValue?.problem || 'Diagramme d\'Ishikawa').toString().substring(0, 80);
        const exportLocale = new Date().toLocaleString('fr-FR');
        applyJsPdfHeader(pdf, branding, pdfWidth, { titleText, exportLocale, titleY: 10, dateY: 16 });
        applyJsPdfFooter(pdf, branding, pdfHeight, pdfWidth);
    ...
    async exportJSON() {
        ...
        const exportData = {
            metadata: enrichMetadata(metadataBase, branding),
