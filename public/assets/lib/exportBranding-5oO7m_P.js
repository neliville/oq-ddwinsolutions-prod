/**
 * Miroir ESM de public/js/export-branding.js pour Stimulus / React.
 */
...
export function getUserLines(branding) {
  const u = branding?.user || {};
  return [u.displayName, u.jobTitle, u.companyName].filter((s) => s && String(s).trim() !== '');
}

export function measureHeaderExtraHeight(lineCount) {
  return Math.max(0, lineCount) * LINE_HEIGHT;
}

export function computeHeaderHeight(branding, baseHeight = 108) {
  return baseHeight + measureHeaderExtraHeight(getUserLines(branding).length);
}

export function paintStandardExportHeader(ctx, canvasWidth, headerHeight, options) {
  const {
    titleText,
    exportLocale,
    descriptionText = '',
...
export function paintCanvasHeaderExtras(ctx, canvasWidth, yStart, branding) {
...
export function buildBrandedExportCanvas(capturedCanvas, options) {
  const {
    branding,
    titleText,
    exportLocale,
...
export function paintCanvasFooter(ctx, canvasWidth, canvasHeight, footerHeight, branding) {
...
export function paintWatermark(ctx, x, y, branding) {
  ctx.save();
  ctx.translate(x, y);
...
export function applyJsPdfHeader(pdf, branding, pageWidth, options) {
...
export function applyJsPdfFooter(pdf, branding, pageHeight) {
  const customFooter = branding?.user?.pdfFooter;
  ...
  if (customFooter) {
    pdf.setFontSize(7);
    pdf.text(customFooter, 10, pageHeight - 10);
    pdf.setFontSize(8);
  }
  pdf.text(copyright, 10, pageHeight - 5);
}

export function enrichMetadata(metadata, branding) {
...
export async function loadExportBranding() {
...
export function invalidateExportBrandingCache() {
  cache = null;
  loadPromise = null;
  if (typeof window !== 'undefined' && window.OqExportBranding?.invalidateCache) {
    window.OqExportBranding.invalidateCache();
  }
}

export { LINE_HEIGHT, SYSTEM_DEFAULTS };
